        ...
        try:
            for info in p:
                ...
                if r not in buff and info.find(class_='cell c3').text[0:3] == 'Зав':
                    ...
                    buff.append(r)
                    clas = f'cell c7 bold'
                    if info.find(class_=clas).text[0] == '-':
                        nesdali.append(r)
                        continue
                    if float(info.find(class_=clas).text.split('/')[0].replace(',', '.'))<maxball:
                        nevipoln[r] = info.find(class_=clas).text.split('/')[0]
                    try:
                        dict[r]+=float(info.find(class_=clas).text.split('/')[0].replace(',', '.'))
